...
const Faq = () => {
  ...
  const getFaqsList = async () => {
    const {
      ...
      message,
    }: any = await Api.get("crm_get_tds_info", {"type":"tds_info"});
    if (responseStatus) {
      setFaqsData(data?.data);
      setOriginalFaqsData(data?.data);
    } else {
      setFaqsData([]);
    }
  };
  ...
  const createFaqs = async (payload: any) => {
    try {
      ...
      const {
        ...
        status: responseStatus,
        message,
      }: any = await Api.post("crm_post_tds_info", payload);
      if (responseStatus) {
    ...
    <>
      <div className="tw-pl-2 tw-pr-6 tw-pb-6">
        <div className="tw-flex tw-flex-col tw-gap-4">
          <p className="tw-text-2xl tw-font-bold tw-text-black tw-mt-5">TDS Information</p>
          ...
          <Table aria-label="simple table">
            ...
            <TableBody>
              {faqsData?.map((row: any, index: any) => (
                <TableRow  key={index} className="tw-bg-[#FFFFFF]">
                  <TableCell>
                    <p className="tw-font-bold tw-flex tw-justify-center tw-text-[#2F4F4F]">{row?.description}</p>
                  </TableCell>
                  <TableCell>
                    <IconButton
                      aria-label="edit"
        ...
        <Modal open={open} onClose={handleClose}>
          <Box sx={modalContentStyle}>
            <form onSubmit={handleSubmit(onSubmit)}>
              <div className="tw-flex tw-justify-between">
                <p className="tw-font-bold tw-text-2xl tw-text-black">{!isEditMode ? `Create TDS` : `Update TDS`}</p>
