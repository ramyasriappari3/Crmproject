...
interface ReservedSlotsPageProps {
...
const ReservedSlotsPage: React.FC<ReservedSlotsPageProps> = ({ parkingSlots, parkingLocation, onBack }) => {
    const [selectedSlots, setSelectedSlots] = useState<string[]>([]);
    const [isConfirmReserve, setIsConfirmReserve] = useState<boolean>(false);
    ...
    const onConfirmReserve = async () => {
        ...
        const slotsToUpdate = selectedSlots.map(slotCode => {
            ...
            return {
                ...
                parking_slot_status: 'reserved' as SlotStatus
            };
        });
        ...
        return sortedSlots.map((slot, index) => {
            ...
            return (
                <button
                    ...
                    title={
                        status === 'booked_by_customer'
                            ? ` ${slot.full_name || 'N/A'} - ${slot.unit_no || 'N/A'} - ${slot.customer_number || 'N/A'}`
                            : `${slot.location_desc || ''} - ${slot.car_parking_slot_number || ''}`
                    }
                >
    ...
    return (
            ...
            {isConfirmReserve && (
                <SlotBookingPopup 
                    ...
                    onCancel={() => setIsConfirmReserve(false)} 
                    isUnreserving={false}
            ...
            <div className="tw-mt-4 tw-flex tw-justify-between">
                {selectedSlots.length > 0 && (
                    <button 
                        onClick={() => setIsConfirmReserve(true)} 
                        className="tw-bg-green-500 tw-text-white tw-py-2 tw-px-4 tw-rounded"
                    >
